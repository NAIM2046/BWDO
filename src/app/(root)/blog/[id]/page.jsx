    ...
    <main className="min-h-screen bg-gray-50 text-gray-800">
      ...
      <div className="max-w-6xl mx-auto px-4 sm:px-6 lg:px-8 pt-10">
        <nav className="text-sm text-gray-500 mb-6">
          ...
          <span className="mx-2">/</span>
          <span className="text-gray-700">{demoPost.title}</span>
        </nav>

        {/* Cover Image */}
        <div className="rounded-2xl overflow-hidden shadow-lg mb-8">
          <Image
            src={demoPost.coverImage}
            alt="Cover"
             width={1200}
    height={600}
        ...
        <header className="text-center">
          ...
          <div className="flex flex-col sm:flex-row justify-center items-center gap-4 text-sm text-gray-500">
            <div className="flex items-center gap-3">
              <img
                src={demoPost.author.avatar}
                alt={demoPost.author.name}
                className="w-12 h-12 rounded-full border border-gray-200 object-cover"
              />
              <div className="text-left">
                <div className="font-medium text-gray-800">{demoPost.author.name}</div>
                <div>{demoPost.author.role}</div>
              </div>
            </div>
            ...
            <div>
              <span>{demoPost.date}</span> <span className="mx-2">•</span>
              <span>{demoPost.readTime}</span>
            </div>
          </div>
        ...
        <section className="mt-12 max-w-4xl mx-auto prose prose-blue prose-lg">
          {demoPost.content.map((block, idx) => {
            switch (block.type) {
              case "heading":
                return (
          ...
          {/* Video Section */}
          <div className="mt-12">
            <h3 className="text-xl font-semibold mb-3">Watch: Project Highlights</h3>
            <div className="aspect-video rounded-xl overflow-hidden shadow-lg">
              <iframe
                src={demoPost.videoUrl}
                title="Project Video"
